...
///   ) query;
typealias GetTransactionsFn = CandidFunctionSignature
...
/// };
typealias Nft_Canister = CandidServiceSignature
...
///   EscrowRecord__1;
/// };
typealias StableEscrowBalances = [UnnamedType35]
/// type StableNftLedger = vec record { text; TransactionRecord };
typealias StableNftLedger = [UnnamedType36]
/// type StableOffers = vec record { Account; Account; int };
typealias StableOffers = [UnnamedType37]
...
///   EscrowRecord__1;
/// };
typealias StableSalesBalances = [UnnamedType35]
...
enum SaleInfoRequest: Codable {
    ...
    case active(UnnamedType31?)
    case deposit_info(Account?)
    case history(UnnamedType31?)
...
enum SaleInfoResponse: Codable {
    ...
    case active(eof: Bool, records: [UnnamedType32], count: BigUInt)
...
enum StreamingStrategy: Codable {
    case Callback(token: StreamingCallbackToken, callback: CandidFunctionSignature)
...
struct TokenIDFilter: Codable {
    let filter_type: UnnamedType39
    let token_id: String
    let tokens: [UnnamedType40]
}
...
struct TransferResultItem: Codable {
    let token_id: BigUInt
    let transfer_result: UnnamedType42
}
...
class GoldNFTService {
    ...
    func dip721_transfer_from(_ arg0: CandidPrincipal, _ arg1: CandidPrincipal, _ arg2: BigUInt, sender: ICPSigningPrincipal? = nil) async throws -> DIP721NatResult {
        let method = ICPMethod(
            ...
            args: .record([
                0: try CandidEncoder().encode(arg0),
                1: try CandidEncoder().encode(arg1),
                2: try CandidEncoder().encode(arg2),
            ])
        )
        let response = try await client.callAndPoll(method, effectiveCanister: canister, sender: sender)
        return try CandidDecoder().decode(response)
    ...
    func mint_batch_nft_origyn(_ arg0: [UnnamedType29], sender: ICPSigningPrincipal? = nil) async throws -> [OrigynTextResult] {
        let method = ICPMethod(
            canister: canister,
    ...
    func stage_batch_nft_origyn(_ arg0: [UnnamedType30], sender: ICPSigningPrincipal? = nil) async throws -> [OrigynTextResult] {
        let method = ICPMethod(
            canister: canister,
    ...
    func stage_nft_origyn(_ arg0: UnnamedType30, sender: ICPSigningPrincipal? = nil) async throws -> OrigynTextResult {
        let method = ICPMethod(
            canister: canister,
